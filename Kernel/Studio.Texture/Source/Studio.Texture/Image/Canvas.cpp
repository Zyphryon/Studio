...
namespace Studio::Texture
{
    ...
    void Canvas::Blit(ConstRef<Canvas> Source, UInt32 FromX, UInt32 FromY, UInt32 Width, UInt32 Height, UInt32 ToX, UInt32 ToY)
    {
        ...
        // Both ends share one layout, so each row of the rectangle moves as one block.
    ...
    Canvas Canvas::From(ConstRef<Bitmap> Source)
    {
        ...
        // Only the base level is read, so the copy the transcoder consumes stops where the level does.
        ...
        Bitmap Floats = Transcoder::Transcode(Move(Level), kFormat);
    ...
    Bitmap Canvas::To(ZyGraphic::TextureFormat Format) const
    {
        ...
        Bitmap Floats(kFormat, mWidth, mHeight, 1, Blob::Copy(ConstSpan<Byte>(mPixels.GetData<Byte>(), mPixels.GetSize())));
